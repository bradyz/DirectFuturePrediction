...
class Agent:
    def __init__(self, sess, args):
        ...
        # preprocessing
        # NOTE(bradyz): preprocessing is now done by sensors.py
        ...
        self.sensor_args = args['sensor_args']
        ...
        self.init_model = os.path.join(self.checkpoint_dir, self.model_dir)
    ...
    def build_model(self):
        ...
        self.input_images_preprocessed = self.input_images
        ...
        # make the actual net
        self.pred_all, self.pred_relevant = self.make_net(
                self.input_images_preprocessed, self.input_measurements_preprocessed,
    ...
    def train(self, simulator, experience, num_steps, test_policy_experience=None):
        ...
        self.train_actor = self.get_actor(
                ...
                random_prob=self.random_exploration_schedule(self.curr_step),
                random_objective_coeffs=self.random_objective_coeffs)

        print('Filling the training memory')
        experience.add_n_steps_with_actor(
                simulator,
                experience.capacity / simulator.num_simulators,
                self.train_actor, verbose=False)
        print('filled')
        ...
        for i in range(num_steps):
            ...
            if self.test_policy_every and np.mod(self.curr_step, self.test_policy_every) == 0:
                self.test_policy(
                        simulator, test_policy_experience,
                        self.objective_coeffs, self.num_steps_per_policy_test, random_prob=0.)
    ...
    def test(self, simulator, test_policy_experience, num_steps, checkpoint):
        start = time.time()
        count = 0

        indices = list(range(2 * num_steps // 1000 - 1))
        multiples = len(indices) // 100
        indices = [i * 1000 for i in indices if i % multiples == 0]
        ...
        for i in reversed(indices):
            ...
            log_path = os.path.join(checkpoint_dir, 'eval_log.txt')
            checkpoint = os.path.join(checkpoint_dir, 'predictor.model-%d' % i)

            if not os.path.exists(checkpoint + '.meta'):
                print('skip %s' % checkpoint)
                continue

            self.saver.restore(self.sess, checkpoint)

            all_info = self.test_policy_mine(
                    simulator, test_policy_experience,
                    self.objective_coeffs, self.num_steps_per_policy_test)

            tmp = [np.mean(np.array(x)[:,2]) for x in all_info]

            with open(log_path, 'a') as f:
                f.write('%s %s %s %s' % (i, time.time() - cur, np.mean(tmp), np.std(tmp)))
                f.write('\n')

            print(time.time() - cur)


    def test_policy_mine(self, simulator, experience, objective_coeffs, num_steps,
            num_trials=3, num_eval_steps=25000):
        ...
        for _ in range(num_trials):
            ...
            while n < num_eval_steps:
                for meas, rwrds, steps in experience.add_n_steps_with_actor_test(
                        simulator, num_steps, actor):
                    info.append(list(meas) + [rwrds, steps])
                    n += steps
                    print(
                            np.mean(np.array(info)[:,2]),
                            np.std(np.array(info)[:,2]),
                            info[-1], len(info))
    ...
    def test_policy(self,
            simulator, experience, objective_coeffs, num_steps,
            random_prob=0., write_predictions=False):
        ...
        experience.add_n_steps_with_actor(simulator, num_steps, actor, verbose=False, write_predictions=write_predictions, write_logs=True, global_step=self.curr_step*self.batch_size)
