...
class MultiExperiment:
    ...
    def __init__(self, target_maker_args={},
                       ...
                       agent_args={},
                       experiment_args={}):
        self.log_dir = os.path.join(agent_args['log_dir'], agent_args['model_dir'])
    ...
    def run(self, mode):
        if mode == 'show':
            ...
            self.test_policy_experience.log_prefix = os.path.join(self.log_dir, 'log')
            self.ag.train(
            ...
            self.test_policy_experience.log_prefix = os.path.join(self.log_dir, 'log_eval')
            self.ag.test(
                    self.multi_simulator,
                    self.test_policy_experience,
                    self.num_train_iterations,
                    1)
        else:
